...
const sz = 200 * 1024 * 1024; //200 megabytes
const ascii_uppercase = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
const char_chunk_sz = Math.floor(sz/ascii_uppercase.length);

var garbage = "";
for(let i=0; i<ascii_uppercase.length; ++i) {
  garbage += ascii_uppercase[i].repeat(char_chunk_sz);
}
garbage += "\n";
...
function HttpServer(options) {
  ...
  before.push(function (req, res) {
    var url = req.url;
    if(url === '/garbage') {
      res.write("<html><head><title>Garbage</title></head>");
      res.write("<body>");
      res.write(`<p>${garbage}</p>`);
      res.write("</body></html>");
      return res.end();
    }
  ...
  before.push(function (req, res) {
    var url = req.url;
    if(url === '/testpath') {
      res.write("<html><head><title>foobar</title></head>");
      res.write("<body>");
